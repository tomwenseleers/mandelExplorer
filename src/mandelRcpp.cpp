...
arma::mat mandelsmoothRcpp(const double x_min, const double x_max, const double y_min, const double y_max,
  ...
  const double x_step = (x_max - x_min) / res_x;
  const double y_step = (y_max - y_min) / res_y;
  const double horizon = 1.0e12;
  const double horizonsq = horizon*horizon;
  const double loghorizon = log(log(horizon))/log(2.0);
  int r,c;
#pragma omp parallel for shared(ret) schedule(dynamic,1)
  for (r = 0; r < res_y; r++) {
#pragma omp simd    
    for (c = 0; c < res_x; c++) {
      double zx = 0.0, zy = 0.0, new_zx;
      double cx = x_min + c*x_step, cy = y_min + r*y_step;
      int n = 0;
      for (n=0;  (zx*zx + zy*zy < horizonsq ) && ( n < nb_iter ); n++ ) {
        new_zx = zx*zx - zy*zy + cx;
        zy = 2.0*zx*zy + cy;
        zx = new_zx;
      }
      double az = sqrt(zx*zx + zy*zy);
      ret(c,r) = n - log(log(az))/log(2.0) + loghorizon; //* I think this line is wrong as it tries to calculate log of - number *//
    }
  }
  return ret;
}
